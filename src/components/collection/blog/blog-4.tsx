...
const BlogCard: React.FC<BlogCardProps> = ({ cover, title, summary, author, published_at }) => {
  return (
    <div className="flex flex-col lg:flex-row gap-8 p-5 rounded-md bg-gray-100 dark:bg-gray-900 border border-gray-200/70 dark:border-gray-800/80 ">
      <div className="w-full lg:w-2/5 lg:h-full">
        <Image src={cover} alt="post cover" width={1300} height={600} className="rounded aspect-video lg:aspect-auto lg:h-full w-full object-cover" />
      </div>
      <div className="flex-1 flex flex-col space-y-6">
        ...
        <div className="flex items-center gap-x-4">
          <Image src={author.avatar} alt={author.name} width={40} height={40} className="w-10 h-10 object-cover rounded" />
          <div>
            <p className="text-gray-800 dark:text-gray-50 font-semibold">By {author.name}</p>
            <p className="text-sm text-gray-600 dark:text-gray-300">{published_at}</p>
          </div>
        </div>
      </div>
    </div>
...
const posts: Post[] = [
  {
    ...
    summary: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Eius error magni ipsam earum",
  },
  {
    id: 2,
    cover: "/images/working-on-housing-project.jpg",
